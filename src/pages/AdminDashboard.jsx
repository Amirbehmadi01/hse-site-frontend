        ...
        <div style={styles.inner}>
          ...
          <div style={styles.content}>
              ...
              <>
                ...
                {selectedType === "scores" && (
                  <div style={{ marginTop: 18 }}>
                    <h3>نمرات سرپرستان — ماه: {selectedJYear}/{String(selectedJMonth).padStart(2, "0")}</h3>
                    <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 12 }}>
                      <div style={styles.scoreSummaryCard}>
                        <div style={styles.scoreTitle}>تعداد کل عدم انطباق</div>
                        <div style={styles.scoreNumber}>{ncList.length}</div>
                      </div>
                      <div style={{...styles.scoreSummaryCard, background: "#d4edda", borderColor: "#28a745", minWidth: 200, maxWidth: 300}}>
                        <div style={styles.scoreTitle}>بالاترین نمره</div>
                        <div style={{...styles.scoreNumber, color: "#28a745", fontSize: 20}}>{scoresData.length > 0 ? `${maxScore}%` : "-"}</div>
                        {highest.length > 0 && (
                          <div style={{ fontSize: 11, color: "#155724", marginTop: 6, lineHeight: 1.4 }}>
                            {highest.map((h, idx) => (
                              <div key={idx} style={{ marginTop: idx > 0 ? 4 : 0 }}>
                                {h.supervisor.username} ({h.supervisor.subunit || "-"})
                              </div>
                            ))}
                          </div>
                        )}
                      </div>
                      <div style={{...styles.scoreSummaryCard, background: "#f8d7da", borderColor: "#dc3545", minWidth: 200, maxWidth: 300}}>
                        <div style={styles.scoreTitle}>پایین‌ترین نمره</div>
                        <div style={{...styles.scoreNumber, color: "#dc3545", fontSize: 20}}>{scoresData.length > 0 ? `${minScore}%` : "-"}</div>
                        {lowest.length > 0 && (
                          <div style={{ fontSize: 11, color: "#721c24", marginTop: 6, lineHeight: 1.4 }}>
                            {lowest.map((l, idx) => (
                              <div key={idx} style={{ marginTop: idx > 0 ? 4 : 0 }}>
                                {l.supervisor.username} ({l.supervisor.subunit || "-"})
                              </div>
                            ))}
                          </div>
                        )}
                    ...
                    <div style={{ overflowX: "auto" }}>
                      <table style={styles.table} className="bordered-table">
                        <thead>
                          <tr>
                            <th style={styles.cell}>ردیف</th>
                            <th style={styles.cell}>نام سرپرست</th>
                            <th style={styles.cell}>واحد/زیرواحد</th>
                            <th style={styles.cell}>کل موارد</th>
                            <th style={styles.cell}>رفع شده</th>
                            <th style={styles.cell}>رفع نشده</th>
                            <th style={styles.cell}>ناقص</th>
                            <th style={styles.cell}>امتیاز (%)</th>
                          </tr>
                        </thead>
                        <tbody>
                          {scoresData.map((row, i) => (
                            <tr key={row.supervisor._id || i}>
                              <td style={styles.cell}>{i + 1}</td>
                              <td style={styles.cell}>{row.supervisor.username}</td>
                              <td style={styles.cell}>{row.supervisor.department || "-"}</td>
                              <td style={styles.cell}>{row.total}</td>
                              <td style={styles.cell}>{row.fixed}</td>
                              <td style={styles.cell}>{row.notFixed}</td>
                              <td style={styles.cell}>{row.incomplete}</td>
            ...
            {activeMenu === "checklists" && (
              <div>
                <h2 style={styles.title}>چک‌لیست‌های ایمنی</h2>
                <p style={{ marginBottom: 12, color: "#555" }}>
                  مورد را انتخاب کنید، سپس بین <strong>تکمیل چک‌لیست جدید</strong> و{" "}
                  <strong>مشاهده چک‌لیست‌های قبلی</strong> یکی را انتخاب نمایید.
                </p>

                <div style={{ display: "flex", flexDirection: "column", gap: 10, maxWidth: 500 }}>
                  {safetyChecklists.map((item) => (
                    <div key={item.id} style={{ width: "100%" }}>
                      ...
                      <div style={{ marginTop: 6, display: "flex", gap: 8, flexWrap: "wrap", justifyContent: "center" }}>
                        ...
                        <button
                          style={styles.actionBtn}
                          onClick={() => navigate(`/checklist/${item.id}/previous`)}
                        >
            ...
            {activeMenu === "responses" && (
              <div>
                <h2>پاسخ‌ها</h2>
                <p>برای مشاهده پاسخ‌ها، به صفحهٔ پاسخ‌ها بروید.</p>
                <button style={styles.actionBtn} onClick={() => navigate("/admin/responses")}>مشاهده پاسخ‌ها</button>
              </div>
            )}

            {activeMenu === "ppe" && (
              <div>
                <h2>تحویل تجهیزات حفاظت فردی</h2>
            ...
            {activeMenu === "users" && (
              <div>
                ...
                <p>برای مدیریت کاربران به صفحهٔ مدیریت کاربران بروید.</p>
                <button style={styles.actionBtn} onClick={() => navigate("/admin/users")}>مدیریت کاربران</button>
              </div>
            )}
            ...
            {!activeMenu && (
              <div style={{ textAlign: "center", paddingTop: 40 }}>
                ...
                <p>برای شروع یکی از گزینه‌های سمت راست را انتخاب کنید.</p>
              </div>
            )}
          </div>
          ...
          <div style={styles.menu}>
            <button
              style={menuBtn(activeMenu === "nonconformities")}
              onClick={() => {
                setActiveMenu("nonconformities");
                setSelectedType((prev) => prev || "new");
              }}
            >
              عدم انطباق
            </button>
            ...
            <button
              style={menuBtn(activeMenu === "checklists")}
            ...
            <button
              style={menuBtn(activeMenu === "responses")}
              onClick={() => { setActiveMenu("responses"); navigate("/admin/responses"); }}
            >
              پاسخ‌ها {responsesNotifications > 0 && <span style={styles.badge}>{responsesNotifications}</span>}
            </button>

            <button
              style={menuBtn(activeMenu === "ppe")}
              onClick={() => setActiveMenu("ppe")}
            >
              تحویل تجهیزات حفاظت فردی (PPE)
            </button>

            <button
              style={menuBtn(activeMenu === "users")}
